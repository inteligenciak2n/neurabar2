...
### Non-obvious gotchas
- **Brand assets (login logo etc.):** after first clone, run `php artisan storage:link` and `php artisan assets:link`. Without these, `$page.props.assets` (`/storage/assets/...`) 404s and the NeuraBar logo on `/login` is broken. Symlinks are `public/storage → storage/app/public` and `storage/app/public/assets → resources/assets`.
- **Locale:** guest UI follows the browser `Accept-Language` (via `SetLocaleMiddleware`), not only `APP_LOCALE`. Portuguese login copy ("E-mail", "Senha", "Faça o login") comes from `resources/translations/pt/Login.json`. Force via `POST /api/set/locale` with `{"locale":"pt"}` if the browser prefers English.
...
- **DB credentials (from `.env`):** host `127.0.0.1:5432`, user `sail`, password `password`. The `.env` (gitignored) is already configured for native dev (`APP_NAME=NeuraBar`, `APP_LOCALE=pt`, `LARAOWL_ENABLED=false`).
