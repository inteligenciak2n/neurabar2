...
class UpdateTranslations extends Command
{
     ...
     * @var string
     */
    protected $description = 'Atualiza arquivos de tradução mapeando en para pt e es, usando AWS Translate ou copiando textos';
    ...
    public function handle()
    {
        ...
        if ($useAwsTranslate) {
            ...
            if (! $this->awsTranslationService->isConfigured()) {
                $this->warn('⚠️  AWS não configurado. Copiando textos em inglês.');
            } else {
                $this->info('📝 Modo manual ativado. Copiando textos em inglês.');
            }
        }
        ...
        // Verifica se a pasta en existe
        if (! File::exists($enPath)) {
            $this->error('Pasta de traduções em inglês não encontrada: '.$enPath);

            return Command::FAILURE;
        }

        // Cria a pasta pt se não existir
        if (! File::exists($ptPath)) {
            File::makeDirectory($ptPath, 0755, true);
            $this->info('Pasta de traduções em português criada: '.$ptPath);
        }
        ...
        // Mapeia todos os arquivos JSON da pasta en
        $enFiles = File::glob($enPath.'/*.json');

        $this->info('Encontrados '.count($enFiles).' arquivos de tradução em inglês.');

        $ptStats = ['created' => 0, 'updated' => 0, 'keysAdded' => 0];
        $esStats = ['created' => 0, 'updated' => 0, 'keysAdded' => 0];

        foreach ($enFiles as $enFile) {
            $fileName = basename($enFile);
            $ptFile = $ptPath.'/'.$fileName;
            $esFile = $esPath.'/'.$fileName;

            $this->info('Processando arquivo: '.$fileName);

            // Lê o conteúdo do arquivo en
            ...
            if ($useAwsTranslate) {
                ...
                $this->processFileWithAwsTranslate($enData, $ptFile, $esFile, $fileName, $ptStats, $esStats);
            } else {
                // Processa arquivos copiando texto original (modo antigo)
                $this->processLanguageFile($enData, $ptFile, $fileName, 'português', $ptStats);
                $this->processLanguageFile($enData, $esFile, $fileName, 'espanhol', $esStats);
            }
        }
        ...
        $this->info('=== PORTUGUÊS ===');
        $this->info('Arquivos criados: '.$ptStats['created']);
        $this->info('Arquivos atualizados: '.$ptStats['updated']);
        $this->info('Total de chaves adicionadas: '.$ptStats['keysAdded']);
     ...
     */
    private function processFileWithAwsTranslate(array $enData, string $ptFile, string $esFile, string $fileName, array &$ptStats, array &$esStats): void
    {
        try {
            // Coleta textos novos que precisam ser traduzidos
            $newTextsPt = $this->getNewTextsForTranslation($enData, $ptFile);
            $newTextsEs = $this->getNewTextsForTranslation($enData, $esFile);

            $translations = [];
            $allNewTexts = [];

            // Se há textos novos para traduzir
            if (! empty($newTextsPt) || ! empty($newTextsEs)) {
                $this->info('  🔄 Traduzindo textos novos...');

                // Coleta todos os textos únicos que precisam ser traduzidos
                $allNewTexts = array_unique(array_merge($newTextsPt, $newTextsEs));
            ...
            // Processa arquivo português
            if (! empty($newTextsPt)) {
                $translatedTexts = [];
                foreach ($newTextsPt as $key => $text) {
                    $translatedTexts[$key] = $translations['pt']['texts'][array_search($text, $allNewTexts)] ?? $text;
                }
                $this->updateFileWithTranslations($enData, $ptFile, $fileName, 'português', $translatedTexts, $ptStats);
            } else {
                $this->syncExistingFile($enData, $ptFile, $fileName, 'português', $ptStats);
            }
            ...
            if (! empty($newTextsEs)) {
                ...
                $this->updateFileWithTranslations($enData, $esFile, $fileName, 'espanhol', $translatedTexts, $esStats);
            } else {
                $this->syncExistingFile($enData, $esFile, $fileName, 'espanhol', $esStats);
            }
