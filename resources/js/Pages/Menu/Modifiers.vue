    ...
    <AppLayout :title="__('Modifier Groups')">
        ...
        <div class="space-y-4">
            <div class="flex justify-end">
                <AppButton @click="showCreate = !showCreate">{{ __('New Group') }}</AppButton>
            </div>

            <AppCard v-if="showCreate" :title="__('New Modifier Group')">
                <form @submit.prevent="submitCreate" class="space-y-3">
                    ...
                    <div class="flex justify-end gap-2">
                        <AppButton variant="secondary" type="button" @click="showCreate = false">{{ __('Cancel') }}</AppButton>
                        <AppButton type="submit" :loading="createForm.processing">{{ __('Create') }}</AppButton>
                    </div>
                </form>
            </AppCard>
            ...
            <AppCard v-for="group in modifierGroups" :key="group.id" :title="group.name">
                <div class="flex items-center gap-3 mb-3 text-xs text-muted-foreground">
                    <span v-if="group.required" class="rounded-full bg-amber-100 text-amber-700 px-2 py-0.5">{{ __('Required') }}</span>
                    <span v-if="group.multiple_selection" class="rounded-full bg-blue-100 text-blue-700 px-2 py-0.5">{{ __('Multi-select') }}</span>
                </div>
                <ul class="space-y-1">
                    ...
                    <li v-if="!group.options?.length" class="text-sm text-muted-foreground">{{ __('No options yet.') }}</li>
                </ul>
            </AppCard>

            <p v-if="!modifierGroups?.length" class="text-center text-muted-foreground py-8">{{ __('No modifier groups yet.') }}</p>
        </div>
    </AppLayout>
</template>
