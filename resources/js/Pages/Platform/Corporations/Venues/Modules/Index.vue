...
const submit = () => {
    form.post(route('platform.corporations.venues.modules.store', [props.corporation.id, props.venue.id]), {
...
const destroy = (moduleId) => {
    if (confirm(__('Are you sure you want to disable this module?'))) {
        form.delete(route('platform.corporations.venues.modules.destroy', [props.corporation.id, props.venue.id, moduleId]));
    }
};
</script>

<template>
    <PlatformLayout :title="__('Venue Modules')">
        <template #header>
            <h1 class="font-heading text-xl font-bold text-ocean-deep dark:text-gray-100">{{ venue.name }} - {{ __('Modules') }}</h1>
        </template>

        <div class="max-w-3xl space-y-6">
            <form @submit.prevent="submit" class="space-y-4 bg-white rounded-xl shadow-card p-6 dark:bg-gray-800">
                <h2 class="font-heading font-semibold text-ocean-deep border-b pb-2 dark:text-gray-100 dark:border-gray-700">{{ __('Activate Module') }}</h2>
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-ocean-deep mb-1 dark:text-gray-300">{{ __('Quantity') }}</label>
                        <input v-model="form.quantity" type="number" min="1" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100" />
                        <p v-if="form.errors.quantity" class="mt-1 text-xs text-destructive">{{ form.errors.quantity }}</p>
                    </div>
                </div>
