...
const submit = () => {
    form.post(route('corporation.venues.store'));
};
</script>

<template>
    <AppLayout :title="__('Add Venue')">
        <template #header>
            <h1 class="font-heading text-2xl font-bold text-ocean-deep">{{ __('Add Venue') }}</h1>
        </template>
            ...
            <form @submit.prevent="submit" class="space-y-4 bg-white rounded-xl shadow-card p-6">
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <input v-model="form.phone" type="text" class="w-full rounded-md border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary" />
                    </div>
                    <div>
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a :href="route('corporation.venues.index')" class="rounded-md border border-border px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-muted transition-colors">{{ __('Cancel') }}</a>
                    <button type="submit" :disabled="form.processing" class="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 disabled:opacity-60 transition-colors">{{ __('Create Venue') }}</button>
                </div>
            </form>
        </div>
    </AppLayout>
