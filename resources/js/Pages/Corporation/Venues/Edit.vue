...
<template>
    <AppLayout title="Edit Venue">
        <template #header>
            <h1 class="font-heading text-2xl font-bold text-ocean-deep">Edit Venue</h1>
        </template>
        ...
        <div class="max-w-lg">
            <form @submit.prevent="submit" class="space-y-4 bg-white rounded-xl shadow-card p-6">
                ...
                <div class="flex justify-end gap-3 pt-2">
                    <a :href="route('corporation.venues.index')" class="rounded-md border border-border px-4 py-2 text-sm font-medium text-muted-foreground hover:bg-muted transition-colors">Cancel</a>
                    <button type="submit" :disabled="form.processing" class="rounded-md bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-primary/90 disabled:opacity-60 transition-colors">Save</button>
                </div>
            </form>
        </div>
    </AppLayout>
